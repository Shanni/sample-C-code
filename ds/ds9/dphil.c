...
void pickup(phil_t *ps)
{
	...
	global_state->state[n] = HUNGRY;
	
	while (global_state->state[left] == EATING || global_state->state[right] == EATING) {
		pthread_cond_wait(global_state->cv[n], global_state->mon);
...
void putdown(phil_t *ps)
{
	...
	if (global_state->state[left] == HUNGRY) pthread_cond_signal(global_state->cv[left]);

	if (global_state->state[right] == HUNGRY) pthread_cond_signal(global_state->cv[right]);
